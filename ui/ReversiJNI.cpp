...
#include "ReversiJNI.h"  // Generated
#include "Reversi.h"
...
*/
JNIEXPORT jint JNICALL Java_ui_ReversiBoard_StateReversi(JNIEnv *env, jobject thisObj, jint turn) {
	...
	s=othello.flipPieces();
	writeNextStatus(s);
	writeFlip(othello.PiecesToFlip);	
	return result;
}

JNIEXPORT jint JNICALL Java_ui_ReversiBoard_GameReversi(JNIEnv *env, jobject thisObj,jint x, jint y , jint turn) {
	int board[10][10];
	int result = readBoard(board);
	...
	//othello.createDatabase();
	othello.fillPlayers(playerNum,arr);
        cout<<"dsrgregergv"<<gameType<<endl;
	if(gameType==-2 || gameType==-3){
                cout<<"AI"<<endl;
		othello.fillPlayersComputer(gameType);
	}	
